...
export async function connectFreighterWallet() {
  ...
  if (addressResult.error || !addressResult.value) {
    throw new Error(`Could not get wallet address: ${addressResult.error || "unknown error"}`);
  }
...
async function signAndSubmit(transaction) {
  ...
  if (signed.error || !signed.value) {
    throw new Error(`Transaction signature failed: ${signed.error || "unknown error"}`);
  }
...
export async function sendSinglePayment({ fromAddress, toAddress, amountXlm, note }) {
...
async function loadMemoMap(records) {
  const uniqueHashes = [...new Set(records.map((record) => record.transaction_hash))];
  const memoMap = {};
...
export function buildPaymentSummary(payments) {
  ...
  return {
    ...
    latestDate: new Date(sorted[sorted.length - 1].timestampMs).toLocaleDateString(),
  };
}
