...
function getHorizonServer() {
  if (!horizonServer) {
    horizonServer = new Horizon.Server(STELLAR_HORIZON_URL);
  }
...
async function signAndSubmit(transaction) {
  ...
  return getHorizonServer().submitTransaction(signedTx);
}
...
export async function sendSinglePayment({
  ...
  const source = await getHorizonServer().loadAccount(fromAddress);
  ...
  // Also record on-chain via Soroban contract if configured
  if (isSorobanConfigured()) {
    try {
      await recordPaymentOnChain({
        employerAddress: fromAddress,
        employeeAddress: toAddress,
        amountXlm,
        note: note || "",
      });
    } catch (error) {
      console.warn("Failed to record on-chain via Soroban:", error.message);
      // Don't fail the payment if Soroban recording fails
    }
  }
...
export async function sendBatchPayment({ fromAddress, recipients, note }) {
  ...
  // Also record all payments on-chain via Soroban contract if configured
  if (isSorobanConfigured()) {
    try {
      for (const recipient of recipients) {
        await recordPaymentOnChain({
          ...
          amountXlm: recipient.amountXlm,
          note: note || "",
        });
      }
    } catch (error) {
      console.warn("Failed to record batch payments on-chain:", error.message);
      // Don't fail the payment if Soroban recording fails
    }
  }
...
async function loadMemoMap(records) {
  ...
  await Promise.all(
    uniqueHashes.map(async (hash) => {
      try {
        const tx = await getHorizonServer().transactions().transaction(hash).call();
...
/**
 * Get employer reputation scores for all employers in payments
 */
export async function fetchEmployerReputations(payments) {
  if (!isSorobanConfigured()) {
    return {};
  }

  const employers = [...new Set(payments.map((p) => p.from))];
  const reputations = {};

  await Promise.all(
    employers.map(async (employer) => {
      try {
        const rep = await getEmployerReputation(employer);
        reputations[employer] = rep;
      } catch (error) {
        console.warn(`Failed to fetch reputation for ${employer}:`, error);
        reputations[employer] = {
          score: 0,
          paymentCount: 0,
          totalAmount: "0",
...
/**
 * Get full verification data including payments and reputation
 */
export async function getFullVerificationData(address) {
  const payments = await fetchIncomingPayments(address);
  const summary = buildPaymentSummary(payments);
  const reputations = await fetchEmployerReputations(payments);
